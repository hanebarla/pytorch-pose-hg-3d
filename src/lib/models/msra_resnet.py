...
class PoseLSTMResNet(nn.Module):

    def __init__(self, block, layers, heads, timestep, **kwargs):
        self.inplanes = 64
        self.deconv_with_bias = False
        self.heads = heads
        ...
        # lstm
        self.linear1 = nn.Sequential(nn.AvgPool2d(8),
                                     nn.Flatten(start_dim=1),
                                     nn.Linear(2048, 512))
        self.lstm = nn.LSTM(input_size=512, hidden_size=512, batch_first=True)
        self.linear2 = nn.Sequential(nn.Linear(512, 2048))
        self.upsample = nn.Sequential(nn.Upsample(scale_factor=8, mode="nearest"))
...
class Pose3DCNNResNet(nn.Module):
    ...
    def forward(self, xs):
        ...
        for i in range(self.timestep):
            ...
            ret = {}
            for head in self.heads:
                ret[head] = self.__getattr__(head)(z)
            rets.append([ret])
        return rets
...
def get_pose_lstm_net(num_layers, heads, timestep):
    block_class, layers = resnet_spec[num_layers]

    model = PoseLSTMResNet(block_class, layers, heads, timestep)
    model.init_weights(num_layers, pretrained=True)
    return model


def get_pose_conv3d_net(num_layers, heads, timestep):
    block_class, layers = resnet_spec[num_layers]

    model = Pose3DCNNResNet(block_class, layers, heads, timestep)
    model.init_weights(num_layers, pretrained=True)
    return model
