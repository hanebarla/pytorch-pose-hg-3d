...
from models.msra_resnet import get_pose_net, get_pose_lstm_net, get_pose_conv3d_net
...
def create_lstm(opt, timestep):
    if 'msra' in opt.arch:
        print("=> using msra resnet '{}'".format(opt.arch))
        num_layers = int(opt.arch[opt.arch.find('_') + 1:])
        model = get_pose_lstm_net(num_layers, opt.heads, timestep)
        optimizer = torch.optim.Adam(model.parameters(), opt.lr)
    else:
        assert 0, "Model not supported!"
...
def create_conv3d(opt, timestep):
    if 'msra' in opt.arch:
        print("=> using msra resnet '{}'".format(opt.arch))
        num_layers = int(opt.arch[opt.arch.find('_') + 1:])
        model = get_pose_conv3d_net(num_layers, opt.heads, timestep)
        optimizer = torch.optim.Adam(model.parameters(), opt.lr)
    else:
        assert 0, "Model not supported!"
