    ...
    for i, batches in enumerate(data_loader):
        ...
        if split == 'train':
            ...
            for m in model.parameters():
                if m.grad is None:
                    continue
                if torch.sum(torch.isnan(m.grad)).item() != 0:
                    print(m.grad)
                    raise ValueError
            optimizer.step()
        else:
            ...
            output_depth_flip = torch.from_numpy(output_depth_flip).cuda(
                device=opt.device, non_blocking=True)
            out_rets[-1][-1]['hm'] = (out_rets[-1][-1]['hm'] + output_flip) / 2
            out_rets[-1][-1]['depth'] = (out_rets[-1][-1]['depth'] + output_depth_flip) / 2
        ...
        Loss3D.update(loss_3d.item(), batches['input'][-1].size(0))
...
def train_lstm(epoch, opt, train_loader, model, optimizer, timestep):
